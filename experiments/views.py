...
def generate_experiment_pdf(request, slug):
    ...
    name_style = ParagraphStyle('CN', parent=styles['Normal'], fontName='Times-Bold', fontSize=20, alignment=0, leading=22)
    ...
    header_detail_style = ParagraphStyle('HDS', parent=styles['Normal'], fontName='Times-Roman', fontSize=10, leading=14)
    ...
    if logo_path:
        ...
        header_table = Table([[logo_img, h_info]], colWidths=[75, 465])
    ...
    elements.append(HRFlowable(width="100%", thickness=1.5, color=colors.HexColor("#ff2e2e"), spaceAfter=15))
    ...
    elements.append(Paragraph(f"PRACTICAL REPORT: {experiment.name.upper()}", report_title_style))
    elements.append(HRFlowable(width="60%", thickness=0.5, color=colors.grey, spaceAfter=20))

    # --- Content Sections ---
    section_title_style = ParagraphStyle('STS', parent=styles['Normal'], fontName='Times-Bold', fontSize=12, spaceAfter=8, textColor=colors.HexColor("#b91c1c"))
    text_style = ParagraphStyle('TS', parent=styles['Normal'], fontName='Times-Roman', fontSize=11, leading=14, spaceAfter=12)
    ...
    def page_setup(canvas, doc):
        canvas.saveState()
        canvas.setStrokeColor(colors.HexColor("#ff2e2e"))
        ...
        try:
            ...
            if os.path.exists(gmars_logo):
                canvas.drawImage(gmars_logo, A4[0] - 100, 10, width=80, height=80, mask='auto')
        except: pass
        
        canvas.setFont('Times-BoldItalic', 10)
        canvas.drawRightString(A4[0] - 120, 40, "Signature of Examiner")
        canvas.setFont('Times-Roman', 8)
        canvas.drawString(40, 35, f"Generated: {now().strftime('%d-%m-%Y %H:%M')}")
