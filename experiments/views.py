...
# =========================================
# Global font availability mapping
FONT_CONFIG = {
    'Cambria': 'Times-Roman',
    'Cambria-Bold': 'Times-Bold',
    'Cambria-Italic': 'Times-Italic',
    'Nunito': 'Helvetica',
    'Nunito-Bold': 'Helvetica-Bold'
}

def register_fonts():
    """Tries to register Cambria and Nunito fonts from project or system paths."""
    fonts_to_register = {
        'Cambria': [
            os.path.join(settings.BASE_DIR, "static/fonts/cambria.ttc"),
            "C:/Windows/Fonts/cambria.ttc"
        ],
        'Cambria-Bold': [
            os.path.join(settings.BASE_DIR, "static/fonts/cambriab.ttf"),
            "C:/Windows/Fonts/cambriab.ttf"
        ],
        'Cambria-Italic': [
            os.path.join(settings.BASE_DIR, "static/fonts/cambriai.ttf"),
            "C:/Windows/Fonts/cambriai.ttf"
        ],
        'Nunito': [
            os.path.join(settings.BASE_DIR, "static/fonts/Nunito-Regular.ttf"),
        ],
        'Nunito-Bold': [
            os.path.join(settings.BASE_DIR, "static/fonts/Nunito-Bold.ttf"),
        ]
    }

    for font_name, paths in fonts_to_register.items():
        for path in paths:
            if os.path.exists(path):
                try:
                    pdfmetrics.registerFont(TTFont(font_name, path))
                    FONT_CONFIG[font_name] = font_name
                    break # Success
                except Exception as e:
                    print(f"Failed to register {font_name} from {path}: {e}")

register_fonts()

def get_font(font_name):
    """Returns registered font name or the best available fallback."""
    if font_name in FONT_CONFIG and FONT_CONFIG[font_name] == font_name:
        return font_name
    
    # Fallback logic: Use Nunito if Cambria fails, else standard fonts
    if 'Cambria' in font_name:
        if 'Bold' in font_name and FONT_CONFIG.get('Nunito-Bold') == 'Nunito-Bold':
            return 'Nunito-Bold'
        if FONT_CONFIG.get('Nunito') == 'Nunito':
            return 'Nunito'
            
    return FONT_CONFIG.get(font_name, 'Times-Roman')
...
def generate_experiment_pdf(request, slug):
    ...
    addr_style = ParagraphStyle('AD', parent=styles['Normal'], fontName=get_font('Cambria'), fontSize=10, textColor=colors.grey, alignment=0)
    ...
    header_detail_style = ParagraphStyle('HDS', parent=styles['Normal'], fontName=get_font('Cambria'), fontSize=10, leading=14)
    ...
    meta_style = ParagraphStyle('META', parent=styles['Normal'], fontName=get_font('Cambria-Italic'), fontSize=9, textColor=colors.darkslategray, alignment=2)
    ...
    report_title_style = ParagraphStyle(
        ...
        fontName=get_font('Cambria-Bold'), 
        fontSize=14, 
        alignment=1, 
        spaceAfter=15,
    ...
    # --- Content Sections ---
    section_title_style = ParagraphStyle('STS', parent=styles['Normal'], fontName=get_font('Cambria-Bold'), fontSize=14, spaceAfter=10, textColor=colors.HexColor("#b91c1c"), borderPadding=2, borderLeftWidth=3, borderLeftColor=colors.HexColor("#b91c1c"))
    text_style = ParagraphStyle('TS', parent=styles['Normal'], fontName=get_font('Cambria'), fontSize=12, leading=16, spaceAfter=15, alignment=4) # alignment 4 is JUSITFY
    ...
    if obs_table:
        ...
        if table_content:
            ...
            t.setStyle(TableStyle([
                ...
                ('FONTNAME', (0, 0), (-1, 0), get_font('Cambria-Bold')),
                ('FONTNAME', (0, 1), (-1, -1), get_font('Cambria')),
                ('FONTSIZE', (0, 0), (-1, -1), 10),
    ...
    def page_setup(canvas, doc):
        ...
        canvas.setFont(get_font('Cambria-Bold'), 11)
        canvas.drawRightString(A4[0] - 100, 50, "Signature of Examiner")
        
        canvas.setFont(get_font('Cambria'), 9)
